...
# Copied from transformers.models.bert.modeling_bert.BertSelfAttention with Bert->XLMRoberta
...
# Copied from transformers.models.bert.modeling_bert.BertAttention with Bert->XLMRoberta
class XLMRobertaAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False, position_embedding_type=None):
        super().__init__()
        self.self = XLMRobertaSelfAttention(config, is_cross_attention=is_cross_attention, position_embedding_type=position_embedding_type)
...
# Copied from transformers.models.bert.modeling_bert.BertLayer with Bert->XLMRoberta
class XLMRobertaLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            self.crossattention = XLMRobertaAttention(config, is_cross_attention=self.config.add_cross_attention, position_embedding_type="absolute")
...
# Copied from transformers.models.bert.modeling_bert.BertEncoder with Bert->XLMRoberta
...
class XLMRobertaOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class XLMRobertaForCausalLM(XLMRobertaPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from transformers import RobertaTokenizer, XLMRobertaForCausalLM, RobertaConfig
        >>> import torch
...
class XLMRobertaLMHeadModel(XLMRobertaPreTrainedModel):
    #_keys_to_ignore_on_save = [r"lm_head.decoder.weight", r"lm_head.decoder.bias"]
    #_keys_to_ignore_on_load_missing = [r"position_ids", r"lm_head.decoder.weight", r"lm_head.decoder.bias"]
    #_keys_to_ignore_on_load_unexpected = [r"pooler"]
    ...
    def __init__(self, config):
        ...
        self.roberta = XLMRobertaModel(config, add_pooling_layer=False)
        self.cls = XLMRobertaOnlyMLMHead(config)
    ...
    def forward(
        ...
        Returns:
        Example::
            >>> from transformers import BertTokenizer, XLMRobertaLMHeadModel, BertConfig
            >>> import torch
            >>> tokenizer = BertTokenizer.from_pretrained('bert-base-cased')
            >>> config = BertConfig.from_pretrained("bert-base-cased")
            >>> model = XLMRobertaLMHeadModel.from_pretrained('bert-base-cased', config=config)
            >>> inputs = tokenizer("Hello, my dog is cute", return_tensors="pt")
            >>> outputs = model(**inputs)
            >>> prediction_logits = outputs.logits
        """
        ...
        sequence_output = outputs[0]
        prediction_scores = self.cls(sequence_output) # torch.Size([25, 40, 21130])
